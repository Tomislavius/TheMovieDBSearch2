...
public class TVShowsPopularFragment extends TVShowsBaseFragment {

    @Override
    protected void loadMovies() {
        TheMovieDBAPI service = ServiceGenerator.createService(TheMovieDBAPI.class);

        service.getPopularTVShowsResult(BuildConfig.API_KEY, page).enqueue(new Callback<TMDBResponseData>() {
            @Override
